...
const navItems = [
  ...
  { id: "grahams-chores", label: "Graham’s Chores" },
      ...
      <div className="flex flex-col gap-6 px-4">
        {navItems.map(({ id, label }) => (
          <button key={id} onClick={() => scrollToSection(id)} className="group relative flex items-center gap-3">
            ...
            <span
              className={cn(
                "absolute left-6 font-dot-gothic text-[12px] tracking-[0.06em] opacity-0 transition-all duration-200 group-hover:opacity-100 group-hover:left-8 whitespace-nowrap",
